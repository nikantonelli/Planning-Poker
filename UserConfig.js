...
//        if (me.getPanel().down('#cardspace')) { me.getPanel().down('#cardspace').removeAll(false);}
    ...
    */
    ...
    _configureTimer: function(savedValue) {
        if ((this.app._GC[mainConfigName] === undefined) || (this.app._GC[mainConfigName].votingTime === "00:00")) {
            return;
        }
        if (savedValue === undefined) {
        var bits = this.app._GC[mainConfigName].votingTime.split(':');   //Shortcut as it should always be 00:00 format
        this._timerRunning = (bits[0] * 60) + bits[1];
        this.configPanel.down('#countdowntimer').setValue(this.app._GC[mainConfigName].votingTime);
        } else {
            this._timerRunning = savedValue;
            this.configPanel.down('#countdowntimer').setValue(Math.floor(savedValue/60) + ":" + savedValue % 60);
        }
                            ...
                            items: [
                                {
                                    ...
                                    handler: function() {
                                        if ( this.cardSelected === null ){
                                            ...
                                            if (me._timerSave >0 ) {
                                                me._configureTimer(me._timerSave);
                                            }
                                ...
                                },
                                {
                                    ...
                                    width: 100,
                                    handler: function() {
                                        me._timerSave = me._timerRunning;
                                        me._stopTimer();
                                    },
                                    scope: me
                                },
